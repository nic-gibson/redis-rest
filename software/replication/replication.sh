...
# This script set up replication between two Redis Enterprise clusters. If there is an existing replication between the
...
function get_dbs () {
  ...
  if ! RESPONSE=$(curl -f -s "${HOST}/v1/bdbs" -H "Content-Type: application/json" -u "${USERNAME}:${PASSWORD}"); then
...
function get_db_info () {
  ...
  DBINFO=$(echo "${RESPONSE}" | jq -r ".[] | select(.name == \"${DBNAME}\")")
  if [ -z "${DBINFO}" ]; then
...
function get_database_endpoint () {
  ...
  DNS_NAME=$(echo "${DBINFO}" | jq -r ".endpoints | .[] | select(.addr_type == \"internal\") | .dns_name")
  if [ -z "${DNS_NAME}" ]; then
...
function get_database_port () {
  ...
  PORT=$(echo "${DBINFO}" | jq -r ".endpoints | .[] | select(.addr_type == \"internal\") | .port")
  if [ -z "${PORT}" ]; then
...
function get_database_password () {
  ...
  PASSWORD=$(echo "${DBINFO}" | jq -r ".authentication_admin_pass")
  if [ -z "${PASSWORD}" ]; then
...
function build_replication_url () {
  ...
  ENDPOINT=$(get_database_endpoint "${DBINFO}")
  PORT=$(get_database_port "${DBINFO}") 
  PASSWORD=$(get_database_password "${DBINFO}")
...
if [ -z "${SOURCE}" ] || [ -z "${TARGET}" ] || [ -z "${USERNAME}" ] || [ -z "${PASSWORD}" ] || [ -z "${DBNAME}" ]; then
  echo "Usage: $0 <source> <target> <username> <password> <dbname>"   
  exit 1
fi
...
if [ -n "${SOURCE_REPLICAS}" ]; then
  ...
  NEWREPLICAS=$(echo "${SOURCE_DB}" | jq -r "{\"replica_sync\":\"enabled\", \"replica_sources\": [.replica_sources[] | select(.uri != \"${TARGET_REP_URL}\")]}")
  if ! curl -s -X PUT "${SOURCE}/v1/bdbs/$(get_database_id "${SOURCE_DB}")" -H "Content-Type: application/json" -u "${USERNAME}:${PASSWORD}" -d "${NEWREPLICAS}" > /dev/null; then
...
POSTURL="${TARGET}/v1/bdbs/$(get_database_id "${TARGET_DB}")"

echo "${POSTURL}"

if ! RESPONSE=$(curl -s -X PUT "${TARGET}/v1/bdbs/$(get_database_id "${TARGET_DB}")" -H "Content-Type: application/json" -u "${USERNAME}:${PASSWORD}" -d "${NEWREPLICAS}"); then  
  echo "didn't work" 
  echo "{RESPONSE}"
fi
